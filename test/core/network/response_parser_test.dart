  ...
  group('ResponseParser Tests', () {
    ...
    group('extractMessage', () {
      test('extracts message when key exists', () {
        ...
        expect(ResponseParser.extractMessage(raw), equals('Operation successful'));
      });
      ...
      test('returns default message when keys not found', () {
        final raw = {'foo': 'bar'};
        expect(ResponseParser.extractMessage(raw, 'Default'), equals('Default'));
      });
    });
    ...
    group('extractPaginationMeta', () {
      test('extracts meta map when meta key exists', () {
        ...
        expect(ResponseParser.extractPaginationMeta(raw), equals({'page': 1, 'limit': 10}));
      });
      ...
      test('extracts flat page/limit/total pagination details', () {
        ...
        expect(ResponseParser.extractPaginationMeta(raw), equals({
          'page': 2,
          'limit': 20,
          'total': 100,
          'total_pages': 5,

...
// DATASOURCE PROVIDER
// ═══════════════════════════════════════════════════════════
final adminUnitRemoteDatasourceProvider = Provider<UnitRemoteDatasource>((ref) {
...
// REPOSITORY PROVIDER
// ═══════════════════════════════════════════════════════════
final adminUnitRepositoryProvider = Provider<UnitRepository>((ref) {
  final datasource = ref.watch(adminUnitRemoteDatasourceProvider);
...
// ═══════════════════════════════════════════════════════════
final adminUnitListProvider = FutureProvider<List<Unit>>((ref) async {
...
// ═══════════════════════════════════════════════════════════
final adminUnitDetailProvider = FutureProvider.family<Unit, String>((
  ref,
  unitId,
) async {
  final repository = ref.watch(adminUnitRepositoryProvider);
...
class UnitFormNotifier extends StateNotifier<UnitFormState> {
  ...
  Future<bool> createUnit(Unit unit) async {
    ...
    try {
      ...
      _ref.invalidate(adminUnitListProvider);
      return true;
    } catch (e) {
  ...
  Future<bool> updateUnit(String unitId, Unit unit) async {
    ...
    try {
      ...
      state = UnitFormState(savedUnit: savedUnit);
      _ref.invalidate(adminUnitListProvider);
      _ref.invalidate(adminUnitDetailProvider(unitId));
      return true;
    } catch (e) {
      state = UnitFormState(error: e.toString());
      return false;
